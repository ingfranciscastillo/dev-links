...
const USER_URL = "https://api.dribbble.com/v2/user";
...
function redirectToIntegrations(
	...
	if (detail) url.searchParams.set("dribbble_error", detail.slice(0, 200));
...
async function handleCallback(request: Request) {
	...
	if (!clientId || !clientSecret) {
		return redirectToIntegrations(
			"error",
			"Dribbble is not configured on this server",
		);
	}
	...
	if (!userRes.ok || !userJson.login) {
		return redirectToIntegrations(
			"error",
			"Couldn't read your Dribbble profile",
		);
	}
	...
	if (Number.isFinite(limits.integrations)) {
		...
		const alreadyConnected = existing.some((r) => r.provider === "dribbble");
	...
	const config = { access_token: tokenJson.access_token };
...
export const Route = createFileRoute("/api/integrations/dribbble/callback")({
	server: {
		handlers: {
